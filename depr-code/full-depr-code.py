...
## Step 2: Remove Duplicate app_id from steam_valid_apps.csv
## depricated##
import pandas as pd
...
## Step 3: Query Steam AppDetails API to Pull Details for Each Steam App ID, pulling only "game" details
    ## Save  details to JSON (came as JSON), log processed apps in processed_apps.csv
import requests
import pandas as pd
import json
import time
...
def fetch_app_detail(app_id):
    ...
    retries = 3  # Max retries for failures
...
def load_existing_json():
    ...
    if os.path.exists(PROCESSED_CSV):
        try:
            ...
            print(f"⚠️ Error reading processed_apps.csv: {e}")
...
MISSING_APPS_CSV = "missing_apps.csv"  # (Optional) Output file for missing apps
...
# This step should not exist??? Should use the original steam_valid_apps_unique.csv to ensure consistency???
import json
import pandas as pd
